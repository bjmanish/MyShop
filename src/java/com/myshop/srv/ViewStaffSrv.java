...
public class ViewStaffSrv extends HttpServlet {
...
//        check admin validation
        ...
        if(session == null || !"admin".equals(session.getAttribute("usertype"))){
            message = "Session Expired | access_denied.";
            response.sendRedirect("login.jsp?"+message);
            return;
        }
        
        StaffServiceImpl staffDao = new StaffServiceImpl();
        List<StaffBean> staffList =     staffDao.getAllStaffs();   
        System.out.println("staffList: "+staffList.toString());
